...
sdev_df <- lapply(sdev,
                  function(y) data.frame("Sdev" = unlist(y),
                      ...
                      "PC"=1:length(y[[1]])))
sdev_df_ <- do.call(rbind,sdev_df)
sdev_df_$Dataset <- unlist(lapply(seq(dataset), function(x) rep(dataset[x], nrow(sdev_df[[x]]))))
pdf(file="/Users/elise/Desktop/GitHub/Hubness_sc/Figure1_supp/screeplot.pdf")
ggplot(sdev_df_[sdev_df_$PC<30,], aes(x=PC, y=Sdev, color=Dropout)) +
   geom_point() +
...
pdf(file="/Users/elise/Desktop/GitHub/Hubness_sc/Figure1_supp/IDxDropout.pdf")
...
ggplot(linear_dim_df, aes(x=Dropout, y=GID_pca, color=Dataset)) +
  geom_point() +
...
#pdf(file="/Users/elise/Desktop/GitHub/Hubness_sc/Figure3_supp/DropoutxHubness.pdf")
#ggarrange(plotlist=lapply(df,
#                          function(x) ggplot(x, aes(x=Dimension, y=GHubness)) +
...
for (i in seq(n_dataset)) {
   for (method in c("Asymmetry", "Skewness")) {
      ...
      with(d3_df, plot3d(Dimension, ID2, GHubness, col = colorby,
                         xlab = "Embedding dimension", ylab = "Instrinsic dimension", zlab = method))
      ...
      rgl.postscript(paste0("/Users/elise/Desktop/GitHub/Hubness_sc/Figure1_supp/3DHubnessxIDxDropout_",dataset[i],"_",method,".pdf"),"pdf") 
      movie3d(spin3d(), dir="/Users/elise/Desktop/GitHub/Hubness_sc/Figure1_supp/", clean=T, duration=15, movie=paste0("3DHubnessxIDxDropout_",dataset[i],"_",method))
   }
}
