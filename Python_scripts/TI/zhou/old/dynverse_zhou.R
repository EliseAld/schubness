...
dev.off()
pdf(file=paste0("/Users/elise/Desktop/GitHub/Hubness_sc/Python_scripts/TI/zhou/bubbleplot_score_W_",choice,"_sup3.pdf"),
    width=18,height=15) #sup refers to the nb of clusters chosen or not
...
ggplot(scores2[[x]][scores2[[x]]$Hub_reduction!="nothing",], aes(x=X, y=Y)) +
  ...
  geom_rect(aes(ymin=as.numeric(Y), ymax=as.numeric(Y)+(diff_score/0.13*1), xmin=as.numeric(X)-0.4, xmax=as.numeric(X)+0.4, fill = diff_score)) +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
  geom_text(aes(label=round(diff_score,3), y=as.numeric(Y)+(diff_score/0.13*1)/2), size=3) +
  theme(text = element_text(size=12),
        axis.ticks.y = element_blank(),
        panel.background = element_rect(fill="grey98"),
        panel.grid = element_line(colour = "grey80")) +
...
ggplot(scores_jo1, aes(y=featureimp_wcor, x=Dimension, fill=Hub_red)) + #6x10
