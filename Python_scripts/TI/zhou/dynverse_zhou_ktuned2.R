...
# ====
path <- "/Users/elise/Desktop/GitHub/Hubness_sc/data/forTI/zhouDR/datasets/h5_jo_ktuned2/"
...
make_paga_traj <- function(dataset_id) {
  if (file.exists(paste0(path,dataset_id, "_",methods[1],".h5seurat"))) {
    rds_truth = readRDS(file=paste0(gsub("h5_jo_ktuned2","rds2",path),
...
pblapply(settings,
         function(setting) {
           if (!file.exists(paste0(
             "/Users/elise/Desktop/GitHub/Hubness_sc/Python_scripts/TI/zhou/scores_scanpy_ktuned/",
             setting,
             ".rds"))) {
             ...
             saveRDS(scores_result,
                     file=paste0("/Users/elise/Desktop/GitHub/Hubness_sc/Python_scripts/TI/zhou/scores_scanpy_ktuned/",
                                 setting,
                                 ".rds"))
           }
...
pdf(file=paste0("/Users/elise/Desktop/GitHub/Hubness_sc/Python_scripts/TI/zhou/bubbleplot_score_",choice,"_sup_ktuned.pdf"),
...
dev.off()
pdf(file=paste0("/Users/elise/Desktop/GitHub/Hubness_sc/Python_scripts/TI/zhou/bubbleplot_score_",choice,"_sup2_ktuned.pdf"),
    width=18,height=15) #sup refers to the nb of clusters chosen or not
...
dev.off()
pdf(file=paste0("/Users/elise/Desktop/GitHub/Hubness_sc/Python_scripts/TI/zhou/bubbleplot_score_",choice,"_sup3_ktuned.pdf"),
    width=18,height=15) #sup refers to the nb of clusters chosen or not
lapply(seq(length(scores2)),
...
dev.off()
pdf(file=paste0("/Users/elise/Desktop/GitHub/Hubness_sc/Python_scripts/TI/zhou/bubbleplot_score_W_",choice,"_sup3_ktuned.pdf"),
    width=18,height=15) #sup refers to the nb of clusters chosen or not
lapply(seq(length(scores2_w)),
       function(x) {fig=ggplot(scores2_w[[x]][scores2_w[[x]]$Hub_reduction!="nothing",], aes(x=X, y=Y)) +# 6 x 7.5
         facet_wrap(~Metric) +
         geom_rect(aes(ymin=as.numeric(Y), ymax=as.numeric(Y)+Score, xmin=as.numeric(X)-0.4, xmax=as.numeric(X)+0.4, fill = Score)) +
         scale_fill_gradient2(low = "white", mid = "grey80", high="darkslategray4", midpoint=0.275) +
         geom_text(aes(label=round(Score,3), y=as.numeric(Y)+Score/2), size=3) +
         theme(text = element_text(size=12),
               axis.ticks.y = element_blank(),
...
dev.off()
pdf(file=paste0("/Users/elise/Desktop/GitHub/Hubness_sc/Python_scripts/TI/zhou/bubbleplot_score_",choice,"_sup4_ktuned.pdf"),
    width=18,height=15) #sup refers to the nb of clusters chosen or not
lapply(seq(length(scores2)),
       function(x) {fig=ggplot(scores2[[x]], aes(x=X, y=Y)) +
         facet_wrap(~Metric) +
...
overallscore_single <- data.frame("Score"=sapply(seq(nrow(tmp[[1]])),
                              ...
                              "X"=tmp[[1]]$X)
pdf(file=paste0("/Users/elise/Desktop/GitHub/Hubness_sc/Python_scripts/TI/zhou/overallscore_single_ktuned.pdf"),
    width=18,height=15)
...
keep_lowD_datasets <- setdiff(seq(dataset_choice),keep_id_datasets)
scores2_id <- lapply(scores_result,
                  function(x1) {tmp <- lapply(seq(length(x1)),
                                              ...
                                              data.frame("Score"=unlist(tmp),
                                                         ...
                                                         "Dimension"=factor(n_comps, levels=n_comps)[idx])});
                  do.call(rbind,tmp)})
scores2_lowD <- lapply(scores_result,
                     function(x1) {tmp <- lapply(seq(length(x1)),
                                                 function(idx) {tmp<-cbind(compare_byScore(x1[[idx]][keep_lowD_datasets]),compare_allScore(x1[[idx]][keep_lowD_datasets]));
                                                 data.frame("Score"=unlist(tmp),
                                                            "Metric"=rep(colnames(tmp), each=nrow(tmp)),
                                                            "Hub_reduction"=rep(rownames(tmp), times=ncol(tmp)),
                                                            "Dimension"=factor(n_comps, levels=n_comps)[idx])});
...
scores2_id <- lapply(scores2_id,
                  ...
                  for (idx in seq(nrow(tmp))) {
                    tmp$diff_score[idx]<-tmp$diff_score[idx]-tmp$Score[5*(floor((idx-1)/5))+1]};
                  return(tmp)})
scores2_lowD <- lapply(scores2_lowD,
                     function(x) {tmp<-x;
                     tmp$X<-gsub(25,1,
                                 gsub(50,2,
                                      gsub(100,3,
                     ...
                     for (idx in seq(nrow(tmp))) {
                       tmp$diff_score[idx]<-tmp$diff_score[idx]-tmp$Score[5*(floor((idx-1)/5))+1]};
                     return(tmp)})
pdf(file=paste0("/Users/elise/Desktop/GitHub/Hubness_sc/Python_scripts/TI/zhou/bubbleplot_score_",choice,"_sup_id4_ktuned.pdf"),
    width=18,height=15) #sup refers to the nb of clusters chosen or not
...
overallscore_single_id <- data.frame("Score"=sapply(seq(nrow(tmp_id[[1]])),
                                  ...
                                  "X"=tmp_id[[1]]$X)
tmp_lowD <- lapply(seq(length(scores2_lowD)),
                 function(idx) {scores2_lowD[[idx]]$Preprocess <- names(scores2_lowD)[idx];
                 return(scores2_lowD[[idx]][scores2_lowD[[idx]]$Metric=="OverallScore",])})
overallscore_single_lowD <- data.frame("Score"=sapply(seq(nrow(tmp_lowD[[1]])),
                                                    function(cond) mean(sapply(tmp_lowD,
                                                                               function(x) x$Score[cond]))),
                                     "Metric"="OverallScore",
                                     'Hub_reduction'=tmp_lowD[[1]]$Hub_reduction,
                                     "Dimension"=tmp_lowD[[1]]$Dimension,
                                     "X"=tmp_lowD[[1]]$X)
pdf(file=paste0("/Users/elise/Desktop/GitHub/Hubness_sc/Python_scripts/TI/zhou/overallscore_single_id_ktuned.pdf"),
    width=18,height=15)
...
# Fig TI
x=8 # seurat eucli leiden
# Absolute score
ggplot(scores2[[x]], aes(x=Dimension, y=Hub_reduction)) + #6x7.5
  facet_wrap(~Metric) +
  geom_point(aes(size=Score, color = Score)) +
  geom_text(aes(label=round(Score,3)), size=2) +
  scale_size(range = c(0,15)) +
...
# Diff score
ggplot(scores2[[x]][scores2[[x]]$Hub_reduction!="nothing",], aes(x=X, y=Y)) +
  facet_wrap(~Metric) +
  geom_rect(aes(ymin=as.numeric(Y), ymax=as.numeric(Y)+(diff_score/0.55*1), xmin=as.numeric(X)-0.4, xmax=as.numeric(X)+0.4, fill = diff_score)) +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
  geom_text(aes(label=round(diff_score,3), y=as.numeric(Y)+(diff_score/0.55*1)/2), size=3) +
  ...
  xlab("Dimension")
# Weighted diff score
ggplot(scores2_w[[x]][scores2_w[[x]]$Hub_reduction!="nothing",], aes(x=X, y=Y)) +# 6 x 7.5
   facet_wrap(~Metric) +
   geom_rect(aes(ymin=as.numeric(Y), ymax=as.numeric(Y)+Score, xmin=as.numeric(X)-0.4, xmax=as.numeric(X)+0.4, fill = Score)) +
   scale_fill_gradient2(low = "white", mid = "grey80", high="darkslategray4", midpoint=0.275) +
   geom_text(aes(label=round(Score,3), y=as.numeric(Y)+Score/2), size=3) +
   ...
   scale_x_discrete(labels=c("1"="25",
                             ...
                             "4"="500")) +
   xlab("Dimension")
# Make summary figure with all datasets and only ID and only low ID
...
overall_ci_id$Metric <- unname(sapply(overall_ci_id$Preprocess, function(x)
  strsplit(x, " ")[[1]][2]))
overall_ci_lowD <- do.call(rbind,tmp_lowD)
overall_ci_lowD$Metric <- unname(sapply(overall_ci_lowD$Preprocess, function(x)
  strsplit(x, " ")[[1]][2]))
...
overall_ci_lowD <- overall_ci_lowD[!(overall_ci_lowD$Metric=="cosine" & 
                                       overall_ci_lowD$Hub_reduction=="dsl"),]
ggplot(overall_ci, aes(x=Dimension, y=Score, fill=Hub_reduction)) + #3x5
  ...
  geom_point(size=0.8,position=position_jitterdodge(jitter.width = 0.1),
             aes(color=Hub_reduction), fill="black") +
  ylim(c(0.2,0.35)) +
  ...
  theme(panel.background = element_rect(fill="grey98"),
        panel.grid = element_line(colour = "grey80"),
        axis.text = element_text(size=15))
ggplot(overall_ci_id, aes(x=Dimension, y=Score, fill=Hub_reduction)) + #3x5
  geom_boxplot(outlier.shape=NA, alpha=0.6) +
  geom_point(size=0.8,position=position_jitterdodge(jitter.width = 0.1),
             aes(color=Hub_reduction), fill="black") +
  ylim(c(0.2,0.35)) +
  ...
  theme(panel.background = element_rect(fill="grey98"),
        panel.grid = element_line(colour = "grey80"),
        axis.text = element_text(size=15))
ggplot(overall_ci_lowD, aes(x=Dimension, y=Score, fill=Hub_reduction)) + #3x5
  geom_boxplot(outlier.shape=NA, alpha=0.6) +
  geom_point(size=0.8,position=position_jitterdodge(jitter.width = 0.1),
             aes(color=Hub_reduction), fill="black") +
  ylim(c(0.2,0.35)) +
...
stat.test = overall_ci %>%
  ...
  rstatix::t_test(Score ~ Hub_reduction, ref.group = "nothing")
stat.test.id = overall_ci_id %>%
  group_by(Dimension,Metric) %>%
  rstatix::t_test(Score ~ Hub_reduction, ref.group = "nothing")

# Make supp for seurat, leiden with euclidean or cosine (jo's like)
scores_jo <- lapply(scores_result, function(x) {
  tmp2 <- lapply(seq(x), function(y) {
    tmp <- do.call(rbind,x[[y]]);
    tmp$Dimension <- n_comps[y];
    return(tmp)});
  do.call(rbind,tmp2)})
# seurat leiden
scores_jo1 <- rbind(scores_jo$`seurat cosine leiden`, scores_jo$`seurat euclidean leiden`)
scores_jo1$Metric <- rep(c("cosine","euclidean"), each=length(n_comps)*length(dataset_choice)*length(methods))
scores_jo1$Hub_red <- rep(methods, length(n_comps)*length(dataset_choice))
scores_jo1$Dimension <- factor(scores_jo1$Dimension, levels=sort(n_comps,decreasing = T))
scores_jo1 <- scores_jo1[!(scores_jo1$Metric=='cosine' &
                             scores_jo1$Hub_red=="dsl"),]

ggplot(scores_jo1, aes(y=F1_branches, x=Dimension, fill=Hub_red)) + #6x10
