...
def hub_paga_stab(adata, path_out, n_iter=10, bootstrap=0.95):
...
def all_fast(dataset_folder,n_dim):
    path = main_path+'rds/'+dataset_folder
    print('reading '+dataset_folder)
    adata = load(path)
    if n_dim < adata.shape[0]:
        adata = preprocess(adata, n_dim)
        if n_dim < adata.shape[1]:
            path_out = main_path+'h5/'+dataset_folder.replace('.rds', '')+'_'+str(n_dim)+'dims_'
            hub_paga_stab(adata, path_out)
        else:
