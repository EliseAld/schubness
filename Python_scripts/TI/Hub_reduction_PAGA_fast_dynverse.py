...
def preprocess(adata, n_dim):
    sc.pp.recipe_zheng17(adata) # ! remove datasets with less cells than genes!
...
def hub_paga(adata, methods_set,path_out):
    ...
    for method_name in all_adata.keys():
        all_adata[method_name].write_h5ad(filename=path_out+method_name+"2.h5ad") # v2 since iter=1 and fitted param for clutering
...
if __name__ == '__main__':
    ...
    tools = ['dyngen', 'dyntoy', 'prosstt', 'splatter', 'gold', 'silver']
