    ...
    while this_step < max_steps:
        ...
        if clustering_algo == 'louvain':
            if flavor == 'scanpy':
                ...
                part_louvain = louvain.best_partition(graph=G, weight=weights, resolution=this_resolution, random_state=seed)
...
def ti_analysis(adata,
    ...
    for method_name, (hubness, hubness_params) in hubness_methods.items():
        ...
        sc.pp.neighbors(all_adata[method_name], n_neighbors=n_neighbors, use_rep=method_name)
        G, weights = generate_clustering_inputs(X=X,
                                                 ...
                                                 hubness=hubness,
                                                 hubness_params=hubness_params)
        resol, weighted = getNclusters(adata, G, n_clusters=n_clusters, seed=seed, clustering_algo=clustering_algo,
    ...
    for method_name, (hubness, hubness_params) in hubness_methods.items():
        ...
        for iter in tqdm(range(n_iter)):
            ...
            adata_sampled = anndata.AnnData(X[cell_bootstrap][:, feat_bootstrap],
                                            uns=uns)
            ...
            adata_sampled.obsm[method_name] = kneighbors_graph(X2,
                                                            ...
                                                            hubness=hubness,
                                                            hubness_params=hubness_params)
            sc.pp.neighbors(adata_sampled, n_neighbors=n_neighbors, use_rep=method_name)
