---
title: "Reverse coverage-based hub definition (percent) x Dropout"
...
library(pbapply)
hub_nb_dim<- function(data,d,k,hub_scores,cell_nb, percent) {
...
```{r get neighbors nb, include=FALSE}
k=min(100,round(cell_nb/20))
dim_val=c(2,5,10,20,30,40,50,100,500,1000,dim_nb)
percent=70
hub_nb = c()
for (i in 1:length(simul_pca)) {
  hub_nb <- c(hub_nb,pbapply::pbsapply(dim_val, FUN=function(x) hub_nb_dim(simul_pca[[i]],x,k,hub_scores[[i]],cell_nb,percent)))
}
