...
data_hubness <- data_load("/home/externe/cnrs/aregimbeau/dev/elise/slurm/hubness_load_write_data_bulk_splatter.R")
data_pca <- future_lapply(dataset,
                   function(y) future_lapply(dropout_percent,
                     function(x) read.table(paste0(
                        "/shared/projects/mne/elise/bulk/",
                        y,"/splatter_pca/",
                        "simul_dropout",x,
                        "_pca.csv"))))
...
df <- future_lapply(seq(n_dataset),
             function(y) do.call(rbind,future_lapply(seq(length(dropout_percent)),
             function(x) make_merge(data_hubness[[y]][[x]],data_pca[[y]][[x]],10,pcval[[y]],dropout_percent[x],dim_nb[y]))))
df2 <- future_lapply(seq(n_dataset),
             function(y) do.call(rbind,future_lapply(seq(length(dropout_percent)),
...
saveRDS(df,
        file="/home/externe/cnrs/aregimbeau/dev/elise/slurm/hubness_param_splatter_ggplot1.rds")
saveRDS(df2,
        file="/home/externe/cnrs/aregimbeau/dev/elise/slurm/hubness_param_splatter_ggplot1bis.rds")
...
df_supp <- future_lapply(seq(n_dataset),
             function(y) do.call(rbind,future_lapply(seq(length(dropout_percent)),
             function(x) make_merge_supp(data_hubness[[y]][[x]],data_pca[[y]][[x]],10,pcval[[y]],dropout_percent[x],dim_nb[y]))))

df2_supp <- future_lapply(seq(n_dataset),
             function(y) do.call(rbind,future_lapply(seq(length(dropout_percent)),
             function(x) make_merge_supp(data_hubness[[y]][[x]],data_pca[[y]][[x]],10,pcval2[[y]],dropout_percent[x],dim_nb[y]))))
for (i in seq(n_dataset)) {
   df_supp[[i]]$Dimension <- as.numeric(as.character(df_supp[[i]]$Dimension))
   df2_supp[[i]]$Dimension <- as.numeric(as.character(df2_supp[[i]]$Dimension))
}

saveRDS(df_supp,
        file="/home/externe/cnrs/aregimbeau/dev/elise/slurm/hubness_param_splatter_ggplot2.rds")
saveRDS(df2_supp,
        file="/home/externe/cnrs/aregimbeau/dev/elise/slurm/hubness_param_splatter_ggplot2bis.rds")
print("SUCCESS PARAMS SPLATTER")
