...
matrix <- lapply(dataset,
                   function(x) pblapply(dropout_percent,
                                        function(y)
                                        read.table(paste0("/Users/elise/Desktop/GitHub/Hubness_sc/Data_bulk/", x, "/splatter_pca/simul_dropout", y,"_pca.csv"))))
...
get_scores <- function(data,k.val,pc.val,name,dropout) {
  ...
  for (pc in pc.val) {
    ...
    saveRDS(score, file=paste0("/Users/elise/Desktop/GitHub/Hubness_sc/Figure1_bulk/splatter_scores/",name,"/Dropout",dropout,"_kNN_occurence_",pc,"pca_minkow_bis.rds"))
  }
}
lapply(dataset,
