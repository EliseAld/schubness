...
data_in <- lapply(datasets,
                  function(x) sapply(dropout_percent, function(y) paste0("/shared/projects/mne/elise/bulk/", x, "/simulated/simul_dropout", y,".csv")))
data_out <- lapply(datasets,
                  function(x) sapply(dropout_percent, function(y) paste0("/shared/projects/mne/elise/bulk/", x, "/pca/simul_dropout", y,"_pca.csv")))
sdev_out <- lapply(datasets,
                  function(x) sapply(dropout_percent, function(y) paste0("/shared/projects/mne/elise/bulk/", x, "/pca/simul_dropout", y,"_sdev.csv")))
...
data_proj <- future_lapply(pca, function(y) pblapply(y, function(x) t(x$rotation)))
