...
matrix <- pblapply(dataset,
                   function(x) read.table(paste0("/shared/projects/mne/elise/sc/pca/",x,"_pca_readyforhubness.csv")))
print("data loaded!")
...
get_scores <- function(data,k.val,pc.val,name) {
  ...
  for (pc in pc.val) {
      ...
      saveRDS(score, file=paste0("/shared/projects/mne/elise/sc/scores/",name,"_kNN_occurence_",pc,"pca_minkow_bis.rds"))
    }
}
