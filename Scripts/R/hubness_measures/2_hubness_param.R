...
data_pca <- pblapply(dataset,
                     function(x)read.table(paste0(
                        "/shared/projects/mne/elise/sc/pca/",
                                                   x,"_pca_readyforhubness.csv")))
data_sdev <- pblapply(dataset,
                     function(x)read.table(paste0(
                        "/shared/projects/mne/elise/sc/pca/sdev_",
                                                   x,".csv")))
...
df <- do.call(rbind,future_lapply(seq(n_dataset),
             function(x) make_merge(data_hubness[[x]],data_pca[[x]],10,pcval[[x]],dataset[x],dim_nb[x])))
df2 <- do.call(rbind,future_lapply(seq(n_dataset),
...
df_supp <- do.call(rbind,pblapply(seq(n_dataset),
             function(x) make_merge_supp(data_hubness[[x]],data_pca[[x]],10,pcval[[x]],dataset[x],dim_nb[x])))
df_supp$Dimension <- as.numeric(as.character(df_supp$Dimension))
