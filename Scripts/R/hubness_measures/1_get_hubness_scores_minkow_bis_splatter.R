...
matrix <- future_lapply(dataset,
                   function(x) future_lapply(dropout_percent,
                                        function(y)
                                        read.table(paste0("/shared/projects/mne/elise/bulk/", x, "/splatter_pca/simul_dropout", y,"_pca.csv"))))
...
get_scores <- function(data,k.val,pc.val,name,dropout) {
  ...
  for (pc in pc.val) {
    ...
    saveRDS(score, file=paste0("/shared/projects/mne/elise/bulk/",name,"/splatter_scores/Dropout",dropout,"_kNN_occurence_",pc,"pca_minkow_bis.rds"))
  }
}
