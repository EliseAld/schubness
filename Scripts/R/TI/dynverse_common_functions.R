### Common functions for TI score calculus
...
add_cluster_graph2 <- function(dataset, milestone_network, grouping = NULL, explicit_splits = FALSE) {
  ...
  both_directions <- bind_rows(milestone_network %>%
                                 select(from, to) %>%
                                 mutate(label = from, percentage = 0),
                               milestone_network %>% 
                                 select(from, to) %>%
                                 mutate(label = to, percentage = 1))
...
check_milestone_network2 <- function(milestone_ids, milestone_network, allow_self_loops = T) {
  ...
  assertthat::assert_that(is.character(milestone_network$from),
  ...
  if (!allow_self_loops) {
    assertthat::assert_that(!any((milestone_network$from == milestone_network$to) & milestone_network$length > 0))
  }
  check1 <- milestone_network
  if (allow_self_loops) {
    check1 <- check1 %>% filter(from != to)
  }
  check <-
    inner_join(
  ...
  assertthat::assert_that(nrow(check) == 0, msg = "Milestone network should not contain A->B B->A edges")
...
calculate_mapping_milestones2 <- function(dataset, prediction, simplify = TRUE) {
  mapping <- calculate_mapping2(dataset, prediction, "milestones", simplify = simplify)
