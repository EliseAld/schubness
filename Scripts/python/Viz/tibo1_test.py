...
# generate the n-cubes
cube_dim = dict()
for i in [10, 50, 100, 500]:
    cube_dim[i] = np.random.uniform(-1, 1, size=(n_draws, n_points, i))
print('n-cubes generated')

# Evaluate skewness
cube_dim_skew = np.array([[skhubness.Hubness(k=k, metric='euclidean').fit(cube_dim[i][j, :, :]).score() for i in cube_dim.keys()] for j in range(n_draws)])
plt.clf()
box_skew = plt.boxplot(cube_dim_skew, patch_artist=True, labels=[10, 50, 100, 500])
