...
matrix <- read.table("/Users/elise/Desktop/GitHub/Hubness_sc/data/Guo/revcovhub_rm/revcov_pca_readyforhubness.txt")
print("data loaded!")
n_dim = nrow(matrix)
...
get_scores <- function(data,k.val,p.val,pc.val) {
  for (pc in pc.val) {
    ...
    for (p in p.val) {
      ...
      saveRDS(minkow, file=paste0("/Users/elise/Desktop/GitHub/Hubness_sc/results/Guo/revcov/kNN_occurence_",p,"_pca",pc,"_minkow_bis.rds"))
