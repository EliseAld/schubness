...
# Get the scores
get_scores <- function(data,k.val,p.val,pc.val,dataset) {
  for (pc in pc.val) {
    data_pc <- data[1:pc,]
    for (p in p.val) {
      ...
      saveRDS(minkow, file=paste0("/Users/elise/Desktop/GitHub/Hubness_sc/results/",dataset,"/dca/kNN_occurence_",p,"_pca",pc,"_minkow_latent_bis.rds"))
      print(paste0(p, " pval done for latent compo",pc))
    }
  }
}
