...
matrix <- read.table("GSE99254_NSCLC.TCell.S9055.count.labeled.pca_readyforhubness.txt")
print("data loaded!")
...
pc.val <- c(2,5,10,50,100,500,1000,5000,9046)
...
get_scores <- function(data,k.val,p.val,pc.val) {
  for (pc in pc.val) {
    ...
    for (p in p.val) {
      ...
      saveRDS(minkow, file=paste0("kNN_occurence_",p,"_pca",pc,"_minkow_bis.rds"))
