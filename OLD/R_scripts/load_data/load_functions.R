...
histoplot_hub_spec <- function(seurat, slot, id, ylab, xlab) {
  df=data.frame("counts"=slot,
                "hubs"=id)
  ggplot(df,aes(x=counts, fill=hubs)) +
    geom_histogram() +
    ylab(ylab) +
    xlab(xlab)       
}
vlnplot_hub_spec <- function(seurat, feature, cell_type) {
  ...
  gplot +
    ...
    stat_compare_means(aes(label=..p.adj..), method = "t.test", p.adjust.method = "BH", label = "p.signif",
                       comparisons = list(c(ifelse(cell_type=="hub6","antihub","hub"), "normal")))
}
...
plot_hub <- function(seurat, red, hubs) {
  DimPlot(seurat, reduction=red, group.by = hubs)
}
