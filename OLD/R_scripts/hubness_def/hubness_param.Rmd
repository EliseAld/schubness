...
data_hubness_bulk <- data_load("/Users/elise/Desktop/Github/Hubness_sc/R_scripts/load_data/hubness_load_write_data_bulk.R")
data_pca_bulk <- pblapply(dropout_percent,
                     function(x) read.table(paste0("/Users/elise/Desktop/Github/Hubness_sc/bulkRNAseq/bulkandrei/bulk_simul_pca_readyforhubness",x,".csv")))
data_hubness_tcga <- data_load("/Users/elise/Desktop/Github/Hubness_sc/R_scripts/load_data/hubness_load_write_data_simul.R")
data_pca_tcga <- pblapply(dropout_percent,
                     function(x) read.table(paste0("/Users/elise/Desktop/Github/Hubness_sc/bulkRNAseq/BRCA/simul_pca_readyforhubness",x,".csv")))
...
pcval_bulk <- c(2,5,10,20,30,40,50,100,500,999)
pcval_tcga <- c(2,5,10,20,30,40,50,100,500,1000,1212)
size_bulk <- rep(1000,length(dropout_percent))
size_tcga <- rep(1213,length(dropout_percent))
df_bulk <- pblapply(1:length(Dropout),
             function(x) make_merge(data_hubness_bulk[[x]],data_pca_bulk[[x]],50,pcval_bulk,2,Dropout[x],size_bulk[x]))
df_tcga <- pblapply(1:length(Dropout),
             function(x) make_merge(data_hubness_tcga[[x]],data_pca_tcga[[x]],50,pcval_tcga,2,Dropout[x],size_tcga[x]))
df_hub_bulk <- do.call(rbind,df_bulk)
df_hub_tcga <- do.call(rbind,df_tcga)
df <- list("bulk"=df_hub_bulk,
           "tcga"=df_hub_tcga)
df <- lapply(df,
             function(x) {x$Dimension <- as.numeric(as.character(x$Dimension));
             return(x)})
df <- lapply(df,
             function(x) {colnames(x)[colnames(x) == "Dataset"] <- "Dropout";
             return(x)})
# Viz
p1=ggplot(df[[1]], aes(x=Dimension, y=GHubness)) + geom_line(aes(group=Dropout, color=Dropout)) + geom_point(size=0.1) + facet_wrap(~Method, scales = "free") + ylab(NULL)
p2=ggplot(df[[2]], aes(x=Dimension, y=GHubness)) + geom_line(aes(group=Dropout, color=Dropout)) + geom_point(size=0.1) + facet_wrap(~Method, scales = "free") + ylab(NULL)
ggarrange(p1,p2,labels=c("ARHCS4","BRCA_TCGA"), common.legend = T)
