---
title: "How to define hubs? Zheng8"
output:
...
source(file="/Users/elise/Desktop/GitHub/Hubness_sc/R_scripts/load_data/hubness_load_write_data_Zheng8.R")
...
plot_evol_pval <- function(hub_val,method_name,label.y,factor=T) {
  ggplot(hub_val, aes(x=`if`(factor, factor(Dimension), Dimension), y=GHubness, color=factor(p))) +
   geom_point() +
   ylab(label.y) +
   xlab("Dimension") +
...
# Viz
plot_evol_pval(hub_max, "Maximum score across dimensions","Maximum hubness score", factor = F)
```
...
# Viz
plot_evol_pval(hub_musig, expression(paste("Global hubness defined by cells with a score above ",mu,"+3",sigma," across dimensions")),"Hubs count", factor = F)
```
...
# Viz
plot_evol_pval(hub_2k, "Global hubness defined by cells with a score above 2k, across dimensions","Hubs count", factor = F)
```
...
# Viz
plot_evol_pval(hub_skewness, "Skewness across dimensions","Skewness", factor = F)
```
...
# Viz
plot_evol_pval(hub_musigleft, expression(paste("Global hubness defined by cells with a score above ",mu,"+3",sigma,"_l across dimensions")),"Hubs count", factor = F)
```
...
# Viz
plot_evol_pval(hub_antihub, "Antihub occurence across dimensions","Antihubs count", factor = F)
```
...
# Viz
plot_evol_pval(hub_mad, "Global hubness defined by cells with a score above median+3MAD across dimensions","Hubs count", factor = F)
```
...
# Viz
plot_evol_pval(hub_mode, expression(paste("Global hubness defined by cells with a score above mode+3",sigma," across dimensions")),"Hubs count", factor = F)
```